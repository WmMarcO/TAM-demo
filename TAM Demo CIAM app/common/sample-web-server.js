...
app.get('/tokens', oidc.ensureAuthenticated(), async (req, res) => {
  ...
  let idclaims, atclaims;

  try {
    // Await the access token verification and get the full JWT object
    const accessTokenJwt = await oktaJwtVerifier.verifyAccessToken(tokens.access_token, audience);
    const atclaims = Object.entries(accessTokenJwt.claims);
